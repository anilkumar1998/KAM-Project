...
cv2.namedWindow('Trackbars for HLS')
cv2.createTrackbar('UB for H', 'Trackbars for HLS', 12, 180, nothing)
cv2.createTrackbar('LB for H', 'Trackbars for HLS', 7, 180, nothing)
cv2.createTrackbar('UB for L', 'Trackbars for HLS', 40, 255, nothing)
cv2.createTrackbar('LB for L', 'Trackbars for HLS', 30, 255, nothing) 
cv2.createTrackbar('UB for S', 'Trackbars for HLS', 80, 255, nothing)
cv2.createTrackbar('LB for S', 'Trackbars for HLS', 80, 255, nothing)
...
#     cv2.waitKey()
    frame1=cv2.flip(frame2,1).copy()
    frame2=cv2.flip(frame2,1)
    cv2.rectangle(frame2, (300,100), (600,400), (0,255,0), 2)
    cv2.imshow('frame2', frame2)
    frame1=frame1[100:400,300:600]
    ...
    for i in range(0,7):
        for j in range(0,3):
            if j==0:
                c_lower[i][0]=max(cv2.getTrackbarPos('LB for H','Trackbars for HLS'),3)
                c_upper[i][0]=max(cv2.getTrackbarPos('UB for H','Trackbars for HLS'),3)
            elif j==1:
                c_lower[i][1]=max(10,cv2.getTrackbarPos('LB for L','Trackbars for HLS'))
                c_upper[i][1]=max(10,cv2.getTrackbarPos('UB for L','Trackbars for HLS'))
            else:
                c_lower[i][2]=max(10,cv2.getTrackbarPos('LB for S','Trackbars for HLS'))
                c_upper[i][2]=max(10,cv2.getTrackbarPos('UB for S','Trackbars for HLS'))
    ...
    for i in range(0,7):
        if avgColor[i][0]+c_upper[i][0]>180:
            c_upper[i][0]=180-avgColor[i][0]
    ...
    #     cv2.drawContours(approx_image,[approx],-1,(0,255,0),2)
        ...
        if defects is not None:
            ...
            if(end_time-start_time>3):
                ...
                ser.write(bytes(str(number_of_fingers),'UTF-8'))
                start_time=end_time
        else:
            cv2.putText(approx_image, 'No Defects Found', (50,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,255,0))
        
    else:
        cv2.putText(approx_image, 'No Object Detected', (50,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,255,0))
